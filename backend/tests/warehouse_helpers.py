...
def cable_payload(**attributes):
    return ItemCreate(category_key="optical_patch_cord", name="Synthetic cable", attributes={
        "fiber": "MMF", "fiber_category": "OM4", "connector_a": "LC/UPC",
        "connector_b": "LC/UPC", "length_m": "5", "construction": "Duplex",
        "color": uuid.uuid4().hex, **attributes,
    })
...
async def actor(db, role=UserRole.ADMIN, access=UserAccessStatus.APPROVED):
    ...
    db.add_all([user, TelegramIdentity(user=user, user_id=user.id,
        telegram_user_id=uuid.uuid4().int % 10**14, first_name="Synthetic actor"),
        AuthSession(user=user, user_id=user.id, token_hash=hash_session_token(token),
...
async def scenario(db):
    ...
    locations = [await create_location(db, LocationCreate(code=uuid.uuid4().hex,
        name=name, location_type=kind)) for name, kind in
        [("Synthetic warehouse", "WAREHOUSE"), ("Synthetic DC", "DATACENTER")]]
    return user.id, item, locations[0].id, locations[1].id


async def move(db: AsyncSession, scenario, kind, quantity, *, source=None,
               destination=None, key=None, original=None):
    result = await create_movement(db, MovementCreate(movement_type=kind,
        source_location_id=source, destination_location_id=destination,
        client_request_id=key or uuid.uuid4().hex, original_movement_id=original,
        lines=[MovementLineCreate(item_id=scenario[1], quantity=quantity)]),
        actor_user_id=scenario[0], actor_display_name="Synthetic actor")
    return result
