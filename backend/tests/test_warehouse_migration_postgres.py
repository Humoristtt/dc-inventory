...
async def test_baseline_head_empty_downgrade_and_metadata(migration_database):
    ...
    async with engine.connect() as db:
        ...
        columns = (await db.execute(text("SELECT column_name FROM information_schema.columns WHERE table_schema='public'"))).scalars().all()
        assert not {"serial_number", "wwn", "accounting_mode", "holder_user_id", "source_holder_user_id",
                    "destination_holder_user_id", "datasheet_url"}.intersection(columns)
...
@pytest.mark.parametrize("seed,reason", [
    ("INSERT INTO locations (id,code,normalized_code,name) VALUES (gen_random_uuid(),'test','test','test')", "locations contains data"),
    ("INSERT INTO items (id,category_id,name,normalized_name,accounting_mode,status) SELECT gen_random_uuid(),id,'test','test','QUANTITY','ACTIVE' FROM categories WHERE key='sfp'", "items contains data"),
    ("INSERT INTO items (id,category_id,name,normalized_name,accounting_mode,status) SELECT gen_random_uuid(),id,'test','test','SERIAL','ACTIVE' FROM categories WHERE key='nic'", "serial catalog"),
    ("INSERT INTO categories (id,key,display_name,default_accounting_mode,is_system) VALUES (gen_random_uuid(),'custom','Custom','QUANTITY',false)", "custom categories"),
])
...
async def test_downgrade_refuses_populated_v2_without_losing_data(migration_database, domain):
    ...
    async with AsyncSession(engine, expire_on_commit=False) as db:
        if domain == "item":
            ...
            await actor(db)
            await create_location(db, LocationCreate(code="test", name="test", location_type="WAREHOUSE"))
        else:
            s = await scenario(db)
            await move(db, s, "RECEIPT", 3, destination=s[2])
        await db.commit()
        before = [await db.scalar(text(f"SELECT count(*) FROM {table}")) for table in
                  ("items", "locations", "movements", "movement_lines", "stock_balances")]
    assert "downgrade refused" in alembic(url, "downgrade", PREVIOUS, success=False)
    async with engine.connect() as db:
        after = [await db.scalar(text(f"SELECT count(*) FROM {table}")) for table in
                 ("items", "locations", "movements", "movement_lines", "stock_balances")]
        assert before == after
        assert await db.scalar(text("SELECT version_num FROM alembic_version")) == HEAD
    await engine.dispose()
...
async def test_frozen_configuration_matches_current_product_contract():
    ...
    assert frozen["leaves"] == {k: [v[0], v[1], [asdict(a) for a in v[2]]] for k, v in LEAVES.items()}
