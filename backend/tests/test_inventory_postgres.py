...
async def test_reversal_insufficient_stock_is_atomic(warehouse_db, kind):
    db = warehouse_db
    s = await scenario(db)
...
async def test_journal_is_immutable_at_database_level(warehouse_db, table, operation):
    db = warehouse_db
    s = await scenario(db)
...
async def test_stock_database_rejects_nonpositive_balances(warehouse_db, quantity_value):
    db = warehouse_db
    s = await scenario(db)
    with pytest.raises(DBAPIError):
...
async def test_concurrent_issues_cannot_overspend():
