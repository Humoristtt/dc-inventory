...
Сумма `StockBalance` по локациям даёт текущий общий остаток Item. `UserItemCustodyBalance` показывает агрегированное количество, числящееся за сотрудником; это не сведения о конкретном серийном устройстве. В текущем source-контракте runtime role имеет на обе проекции только `SELECT`: прямые `INSERT/UPDATE/DELETE` запрещены. Запись выполняет `SECURITY DEFINER` функция `refresh_warehouse_projection(uuid, uuid)`, которая в той же транзакции заново выводит затронутые stock/custody из immutable journal. Нулевые строки удаляются, отрицательные и превышающие safe-integer диапазон значения запрещены.
...
4. Читает текущие stock/custody для валидации; сериализация runtime-операций обеспечивается уже взятыми item locks, поэтому для записи проекций не требуется broad DML право runtime role.
5. Создаёт `Movement`/`MovementLine`, затем вызывает DB-controlled projection writer для каждого затронутого Item. Writer пересчитывает точное состояние по журналу, а не применяет переданный клиентом delta.
6. Выполняет COMMIT. Ошибка на любом этапе откатывает движение и проекции вместе; повторный вызов projection writer идемпотентен относительно того же immutable journal.
