...
async def api_context(db, enabled=True):
    ...
    async def session():
        async with AsyncSession(connection, expire_on_commit=False,
                                join_transaction_mode="create_savepoint") as request_db:
            yield request_db
    app.dependency_overrides[get_db_session] = session
    users = {}
    for name, role, access in [("admin", UserRole.ADMIN, UserAccessStatus.APPROVED),
        ...
        users[name] = (user.id, {"Cookie": f"{settings.auth_cookie_name}={token}",
...
async def test_read_access_and_actor_scoped_journal(warehouse_db):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        for path in ["/api/inventory/stock", "/api/inventory/locations", "/api/inventory/movements",
                     "/api/inventory/movement-actors", f"/api/inventory/items/{s[1]}/summary"]:
        ...
        assert (await client.get(f"/api/inventory/movements?actor_user_id={s[0]}",
            headers=users["user"][1])).status_code == 403
        assert (await client.get(f"/api/inventory/movements/{other.record.movement.id}",
            headers=users["user"][1])).status_code == 403
        response = await client.get(f"/api/inventory/movements?actor_user_id={s[0]}&category=optics&location_id={s[2]}",
        ...
        assert (await client.get("/api/inventory/movements?since=2026-01-01T00:00:00",
            headers=users["admin"][1])).status_code == 422
...
async def test_mutation_roles_idempotency_and_gate(warehouse_db):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        for kind in ("ISSUE", "RETURN"):
            payload = {"movement_type": kind, "client_request_id": uuid.uuid4().hex,
                "lines": [{"item_id": str(s[1]), "quantity": 2}],
                ("source_location_id" if kind == "ISSUE" else "destination_location_id"): str(s[2])}
            response = await client.post("/api/inventory/movements", headers=users["user"][1], json=payload)
            assert response.status_code == 201, response.text
            assert response.json()["actor_user_id"] == str(users["user"][0])
            replay = await client.post("/api/inventory/movements", headers=users["user"][1], json=payload)
            assert replay.json()["id"] == response.json()["id"]
            for restricted in ("RECEIPT", "TRANSFER", "WRITE_OFF", "CORRECTION", "REVERSAL"):
                assert (await client.post("/api/inventory/movements", headers=users["user"][1],
                    json={**payload, "movement_type": restricted})).status_code == 403
        reversal = f'/api/admin/inventory/movements/{response.json()["id"]}/reversal'
        assert (await client.post(reversal, headers=users["user"][1],
            json={"client_request_id": "reverse"})).status_code == 403
        assert (await client.post(reversal, headers=users["admin"][1],
            json={"client_request_id": "reverse"})).status_code == 201
        payload = {"code": uuid.uuid4().hex, "name": "Test location", "location_type": "DATACENTER"}
        assert (await client.post("/api/admin/inventory/locations", headers=users["user"][1], json=payload)).status_code == 403
        app.state.settings.real_inventory_mutations_enabled = False
        assert (await client.post("/api/admin/inventory/locations", headers=users["admin"][1], json=payload)).status_code == 423
        assert (await client.post(reversal, headers=users["admin"][1], json={"client_request_id": "gate"})).status_code == 423
        assert (await client.post("/api/inventory/movements", headers=users["user"][1], json={
            "movement_type": "RETURN", "destination_location_id": str(s[2]), "client_request_id": "gate",
            "lines": [{"item_id": str(s[1]), "quantity": 1}]})).status_code == 423
