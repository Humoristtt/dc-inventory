...
DELETE FROM email_outbox AS target
USING candidates
WHERE target.id = candidates.id
RETURNING 1
"""
...
async def run_technical_retention_once(
    ...
    *,
    ...
    auth_sessions = await _delete_batch(
        ...
        cutoff=current_time - timedelta(days=settings.auth_session_retention_days),
        batch_size=batch_size,
    )
    telegram_updates = await _delete_batch(
        db,
        statement=_TELEGRAM_UPDATES_DELETE,
        cutoff=current_time - timedelta(days=settings.telegram_update_retention_days),
        batch_size=batch_size,
    )
    notification_outbox = await _delete_batch(
        db,
        statement=_NOTIFICATION_OUTBOX_DELETE,
        cutoff=current_time - timedelta(days=settings.notification_outbox_retention_days),
        batch_size=batch_size,
    )
    email_outbox = await _delete_batch(
        db,
        statement=_EMAIL_OUTBOX_DELETE,
        cutoff=current_time - timedelta(days=settings.email_outbox_retention_days),
        batch_size=batch_size,
    )
    access_decision_callbacks = await _delete_batch(
        db,
        statement=_ACCESS_CALLBACKS_DELETE,
        cutoff=current_time - timedelta(days=settings.access_callback_retention_days),
        batch_size=batch_size,
    )
