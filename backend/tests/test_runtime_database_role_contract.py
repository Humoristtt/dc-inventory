...
def test_permission_install_commits_before_legacy_session_termination() -> None:
    ...
    grants = (root / "backend/scripts/apply_database_permissions.sql").read_text()
    terminator = (root / "backend/scripts/terminate_legacy_worker_sessions.sql").read_text()

    section = compose.split("\n  db-permissions:\n", 1)[1].split(
        "\n  backend:\n", 1
    )[0]
    transaction = "psql -X --single-transaction -v ON_ERROR_STOP=1"
    apply_file = "-f /scripts/apply_database_permissions.sql"
    terminate_command = "psql -X -v ON_ERROR_STOP=1"
    terminate_file = "-f /scripts/terminate_legacy_worker_sessions.sql"

    assert section.count(transaction) == 1
    assert section.count(apply_file) == 1
    assert section.count(terminate_command) == 1
    assert section.count(terminate_file) == 1
    assert section.index(transaction) < section.index(apply_file)
    assert section.index(apply_file) < section.index(terminate_command)
    assert section.index(terminate_command) < section.index(terminate_file)
    assert (
    ...
    assert "pg_terminate_backend(pid, 5000)" in terminator
    assert "legacy worker sessions remain after revocation" in terminator
